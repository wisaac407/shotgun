...
def mapGreasePencilStrokeEditMode():
...
def mapWeightPaintVertexSelection():
...
def mapGestureBorder():
    ...
    kmi = km.keymap_items.new_modal('DESELECT', 'LEFTMOUSE', 'RELEASE', shift=True)
...
# Map Gesture Straight Line
def mapGestureStraightLine():
...
# Map Paint Stroke Modal
def mapPaintStrokeModal():
    km = kc.keymaps.new('Paint Stroke Modal', space_type='EMPTY', region_type='WINDOW', modal=True)
...
# Map Eyedropper Modal Map
def mapEyedropperModalMap():
    km = kc.keymaps.new('Eyedropper Modal Map', space_type='EMPTY', region_type='WINDOW', modal=True)
    
    kmi = km.keymap_items.new_modal('CANCEL', 'ESC', 'PRESS', any=True)
    kmi = km.keymap_items.new_modal('CANCEL', 'RIGHTMOUSE', 'PRESS', any=True)
    kmi = km.keymap_items.new_modal('SAMPLE_CONFIRM', 'RET', 'RELEASE', any=True)
    kmi = km.keymap_items.new_modal('SAMPLE_CONFIRM', 'NUMPAD_ENTER', 'RELEASE', any=True)
    kmi = km.keymap_items.new_modal('SAMPLE_CONFIRM', 'LEFTMOUSE', 'RELEASE', any=True)
    kmi = km.keymap_items.new_modal('SAMPLE_BEGIN', 'LEFTMOUSE', 'PRESS', any=True)
    kmi = km.keymap_items.new_modal('SAMPLE_RESET', 'SPACE', 'RELEASE', any=True)


# Map View3D Fly Modal
def mapView3DFlyModal():
...
# Map View3D Dolly Modal
def mapView3DDollyModal():
...
def mapClip():
    ...
    kmi_props_setattr(kmi.properties, 'name', 'CLIP_PIE_timecontrol_pie')
    kmi = km.keymap_items.new('wm.call_menu_pie', 'W', 'PRESS', shift=True)
    kmi_props_setattr(kmi.properties, 'name', 'CLIP_PIE_reconstruction_pie')
    kmi = km.keymap_items.new('wm.call_menu_pie', 'S', 'PRESS', shift=True)
    kmi_props_setattr(kmi.properties, 'name', 'CLIP_PIE_solver_pie')
    kmi = km.keymap_items.new('wm.call_menu_pie', 'E', 'PRESS')
    kmi_props_setattr(kmi.properties, 'name', 'CLIP_PIE_tracking_pie')
    kmi = km.keymap_items.new('wm.call_menu_pie', 'W', 'PRESS')
    kmi_props_setattr(kmi.properties, 'name', 'CLIP_PIE_clipsetup_pie')
    kmi = km.keymap_items.new('wm.call_menu_pie', 'Q', 'PRESS')
    kmi_props_setattr(kmi.properties, 'name', 'CLIP_PIE_marker_pie')
...
def main():
    ...
    map3DViewGeneric()
    mapGreasePencil()
    mapGreasePencilStrokeEditMode()
    mapFaceMask()
    mapWeightPaintVertexSelection()
    mapPose()
    mapObjectMode()
    mapPaintCurve()
    mapCurve()
    ...
    mapObjectNonmodal()
    map3DView()
    mapView3DGestureCircle()
    mapGestureBorder()
    mapGestureZoomBorder()
    mapGestureStraightLine()
    mapStandardModalMap()
    mapAnimationChannels()
    mapKnifeToolModalMap()
    mapUVEditor()
    mapTransformModalMap()
    mapUVSculpt()
    mapPaintStrokeModal()
    mapMaskEditing()
    mapEyedropperModalMap()
    mapView3DFlyModal()
    mapView3DWalkModal()
    mapView3DRotateModal()
    mapView3DMoveModal()
    mapView3DZoomModal()
    mapView3DDollyModal()
    mapGraphEditorGeneric()
    mapGraphEditor()
    mapImageGeneric()
    mapImage()
    mapNodeGeneric()
